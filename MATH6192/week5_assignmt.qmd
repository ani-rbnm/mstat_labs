...
##### Q1.  Estimate of wIth(w independed tilde hat)


```{r}
get_wIth <- function(data) {
  var(data$y) / (var(data$y) + var(data$x))
}
get_wIth(boot_df)
```
##### Q2. Standard error of estimate
...
get_wth_samples <- function(data, B) {
  ...
  for (i in seq_len(B)) {
    w_ths[i] <- get_wIth(get_BS_sample(data))
  }
  w_ths
}
```
